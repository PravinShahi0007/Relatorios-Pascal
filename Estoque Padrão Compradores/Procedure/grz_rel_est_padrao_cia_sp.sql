/*------------------------------------------------------------------------
  Procedure.: grz_rel_est_padrao_cia_sp
  Empresa...: Grazziotin S/A
  Finalidade: Gravação e coleta das dados do estoque padrao da CIA

  Autor   Data     Operacao  Descricao
  ??????? ???/???? Criacao
  Antonio JUL/2021 Alteracao Formatacao do codigo-fonte.
  Antonio JUL/2021 Alteracao Alteracao no tamanho das variaveis dos
                             codigos inicial e final de 11 para 15
                             posicoes.
  ...
  Parametros: Empresa#Grupo#DataInicial#DataFinal#DataReferencia#
              PeriodoDiretaInicial#PeriodoDiretaFinal#Mascara#
              CodigoInicial#CodigoFinal#ListarPor?#PorUnidade?#
              NotasRecebidas?#Usuario#
------------------------------------------------------------------------*/
create or replace procedure grz_rel_est_padrao_cia_sp(pi_opcao in varchar2)
is
begin
     declare
            v_result            integer;
            ...
            cursor c_est_venda is
                   ...
                   and a.cod_oper       in (4300,4302,4305,4106,4107)
            ...
            cursor c_est is
                   select substr(b.cod_completo,1,wqtd_casas) cod_completo
                          ...
                          ,decode(pi_lista_unidade,1,uni.cod_unidade_de,pi_grupo) cod_unidade
                   ...
                   and uni.cod_unidade_para = a.cod_unidade
                   and exists (select 1
                               from ge_grupos_unidades ge
                               where (a.cod_unidade=ge.cod_unidade
                                      or a.cod_unidade in (select uni.cod_unidade_para
                                                           from grz_lojas_unificadas_cia uni
                                                           where uni.cod_unidade_de = ge.cod_unidade))
                   ...
                   group by substr(b.cod_completo,1,wqtd_casas)
                            ...
                            ,decode(pi_lista_unidade,1,uni.cod_unidade_de,pi_grupo);
            r_est c_est%rowtype;
            ...
            cursor c_est_direta is
                   ...
                   and exists (select 1
                               from ge_grupos_unidades ge
                               where (d.cod_unidade=ge.cod_unidade
                                      or d.cod_unidade in (select uni.cod_unidade_para
                                                           from grz_lojas_unificadas_cia uni
                                                           where uni.cod_unidade_de = ge.cod_unidade))
                               and ge.cod_grupo = pi_grupo
                               and ge.cod_emp = pi_empresa)
                   and uni.cod_unidade_para = d.cod_unidade
                   ...
                   and exists (select 1
                               from ge_grupos_unidades ge
                               where (o.cod_unidade=ge.cod_unidade
                               or o.cod_unidade in (select uni.cod_unidade_para
                                                    from grz_lojas_unificadas_cia uni
                                                    where uni.cod_unidade_de = ge.cod_unidade))
                               and ge.cod_grupo = pi_grupo
                               and ge.cod_emp = pi_empresa)
                   and exists (select 1 from ge_grupos_unidades ge_lj -- exists para nao entrar o cd no select quando pi_grupo = 810, 830 etc
                               where o.cod_unidade=ge_lj.cod_unidade
                               and ge_lj.cod_grupo in (1912,1932,1942,1952,1972)
                               and ge_lj.cod_emp = pi_empresa)
                   and uni.cod_unidade_para = o.cod_unidade
            ...
            begin
                 ...
                 begin
                      ...
                      end if; -- (r_est_venda.cod_completo <> wcompleto_ant)
                      ...
                      insert into grzw_rel_estoque_padrao (des_usuario
                                                           ...
                                                           ,des_regiao)
                      values (pi_usuario
                              ,r_est_venda.cod_completo
                              ,wdes_nivel
                              ,pi_nivel_mascara
                              ,r_est_venda.cod_editado
                              ,pi_dta_ref
                              ,r_est_venda.cod_unidade
                              ,wdes_unidade
                              ,r_est_venda.qtd_venda
                              ,r_est_venda.vlr_custo
                              ,0,0,0,0,0,0
                              ,wcod_regiao
                              ,wdes_quebra_regiao);
                 end; -- while c_est_venda%found loop
                 fetch c_est_venda into r_est_venda;
                 end loop; -- while c_est_venda%found loop
                 close c_est_venda;
                 commit;
                 ...
                 begin
                      ...
                      end if; -- (r_est.cod_completo <> wcompleto_ant)
                 ...
                 begin
                      ...
                      values (pi_usuario
                              ...
                              ,wcod_regiao
                              ,wdes_quebra_regiao);
                 end; -- c_est_direta%found loop
                 fetch c_est_direta into r_est_direta;
                 end loop; -- c_est_direta%found loop
                 close c_est_direta;
                 commit;
                      ...
                      begin
                           ...
                           insert into grzw_rel_estoque_padrao (des_usuario
                            ...
                            values (pi_usuario
                                    ...
                                    ,wcod_regiao
                                    ,wdes_quebra_regiao);
                      end; -- c_niveis%found loop
                      fetch c_niveis into r_niveis;
                      end loop; -- c_niveis%found loop
                      close c_niveis;
                      commit;
