	 ...
	 r_notas_serv_mvto c_notas_serv_mvto%ROWTYPE;
       ...
       cursor c_venda_mvto is
         ...
         r_receb_juro_mvto c_receb_juro_mvto%ROWTYPE;
          ...
          BEGIN
	       ...
	       pi_conta_cb := '467,438,439,478,479,556';
	       pi_conta_nt := 229;
	  ...
	  BEGIN
	     ...
	     begin
	         ...
	         wValor_lcto := 0;
	         wValor2 := 0;
	         if r_venda_mvto.cod_oper = 300 then
	        	 wValor_lcto := nvl(r_venda_mvto.vlr_venda,0) - nvl(r_venda_mvto.vlr_entrada,0);
	        	 if ((r_venda_mvto.cod_portador = 2900) or (r_venda_mvto.cod_portador = 2910) or
	        	     (r_venda_mvto.cod_portador = 2920) or (r_venda_mvto.cod_portador = 2930) or
	        	     (r_venda_mvto.cod_portador = 2940) or (r_venda_mvto.cod_portador = 2950))then
	        	    	select sum(a.vlr_lcto) as vlr_lcto
	        	    	  INTO vlr_cartao
	        	    	  from grz_tef_transacao_lojas a
	        	    	    where a.cod_emp = pi_rede
	        	    		  ...
	        	    		  and a.num_cupom = r_venda_mvto.num_nota;

	        	       wValor2 := nvl(vlr_cartao,0);
	        	 end if;
	        	 wNum_equipamento := 10;
	        	 wValor_lcto := nvl(wValor_lcto,0) - nvl(wvalor2,0);
	         elsif r_venda_mvto.cod_oper = 302 or r_venda_mvto.cod_oper = 305 then
