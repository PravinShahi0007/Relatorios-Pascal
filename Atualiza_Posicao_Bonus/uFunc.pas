...
function fHextoInt(hexa:char):integer;
var fHextoInts : integer;
begin
 fhextoints := 0;
 try fHextoInts:=strtoint(hexa);
  except on E:exception do
  begin
   if Upcase(hexa)='A' then fHextoInts:=10 else
   if Upcase(hexa)='B' then fHextoInts:=11 else
   if Upcase(hexa)='C' then fHextoInts:=12 else
   if Upcase(hexa)='D' then fHextoInts:=13 else
   if Upcase(hexa)='E' then fHextoInts:=14 else
   if Upcase(hexa)='F' then fHextoInts:=15;
  end;
 end;
 result := fHextoInts;
end;
...
Function Extenso(Mnum:currency):String;
  ...
  Function NrBaixos(mNumB : Integer):String;
   Const mStrNumB: array[0..18] of string =
...
begin
   ...
   begin
     Month := Month + 1;
     if Day < 29 then
       aResult := EncodeDate(Year,Month,Day)
       else
       ...
       begin
         ...
         begin
           try
             aResult := EncodeDate(Year,Month,DayAux);
             break;
           except
             DayAux := DayAux - 1;
           end;
...
LETRAS_PERMITIDAS = ['A'..'Z', 'a'..'z', 'Á', 'À', 'Ã', 'Â', 'Ä', 'É', 'È', 'Ê',
   'Ë', 'Í', 'Ì', 'Î', 'Ï', 'Ó', 'Ò', 'Ô', 'Õ', 'Ö', 'Ú', 'Ù', 'Û', 'Ü', 'Ñ', 'Ç',
   'Ý', 'Ÿ', '&', 'á', 'à', 'ã', 'â', 'ä', 'é', 'è', 'ê', 'ë', 'í', 'ì', 'î', 'ï',
   'ó', 'ò', 'õ', 'ô', 'ö', 'ú', 'ù', 'û', 'ü', 'ç'];
