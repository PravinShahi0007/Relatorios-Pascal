...
begin
     ...
     cursor c_seguro is
            select c.cod_grupo -- Select Dani Cre
                   ,c.cod_quebra
                   ,b.cod_unidade
                   ,sum(decode(a.cod_oper,300,0,4300,0,6100,0,nvl(a.vlr_operacao,0) - nvl(a.VLR_ENTRADA,0))) vlr_venda
                   ,count(distinct (decode(a.cod_oper,302,a.num_seq,305,a.num_seq,4302,a.num_seq,4305,a.num_seq))) qtd_venda
                   ,nvl(sum(decode(trim(b.num_modelo),'90',nvl(a.vlr_operacao,0),0)),0) vlr_seguro
                   ,count(distinct (decode(a.cod_oper,6100,a.num_seq))) qtd_seguro
            ...
            and b.dta_emissao >= to_date(pi_dta_ini)
            and b.dta_emissao <= to_date(pi_dta_fim)
            ...
            select c.cod_grupo -- Select CPP Jaisson
                   ,c.cod_quebra
                   ,b.cod_unidade
                   ,sum(decode(a.cod_oper,6110,0,nvl(a.vlr_operacao,0) - nvl(a.VLR_ENTRADA,0))) vlr_venda
                   ,count(distinct (decode(a.cod_oper,3000,a.num_seq,3050,a.num_seq))) qtd_venda
                   ,nvl(sum(decode(trim(b.num_modelo),'90',nvl(a.vlr_operacao,0),0)),0) vlr_seguro
                   ,count(distinct (decode(a.cod_oper,6110,a.num_seq))) qtd_seguro
            from ns_notas_operacoes a,
                 ns_notas b,
                 ge_grupos_unidades c
            where a.num_seq      = b.num_seq
     ...
     begin -- Inicio da procedure principal
          ...
          begin
               ...
               begin
                    ...
                    and op.cod_oper = 6110
                    and nc.num_apolice = 0000100
                    and t.cod_unidade =	 r_seguro.cod_unidade
                    and trunc(dta_cancelamento) >= to_date(pi_dta_ini)
                    and trunc(dta_cancelamento) <= to_date(pi_dta_fim);
                    exception
                             when no_data_found then
                                  wVlr_cancelamento  := 0;
                                  wQtd_cancelados    := 0;
               ...
               if pi_CPP = 'CPP' then
               begin
                    select sum(decode(a.cod_oper,6110,0,nvl(a.vlr_operacao,0) - nvl(a.vlr_entrada,0))) vlr_venda
                           ,count(distinct (decode(a.cod_oper,3000,a.num_seq,3050,a.num_seq))) qtd_venda
                           ,nvl(sum(decode(trim(b.num_modelo),'90',nvl(a.vlr_operacao,0),0)),0) vlr_seguro
                    ...
                    and c.cod_grupo   = pi_grupo
                    and b.cod_unidade = r_seguro.cod_unidade
                    and b.dta_emissao >= to_date(pi_dta_inimes)
                    and b.dta_emissao <= to_date(pi_dta_fimmes)
                    and not exists (select 1 from  grz_lojas_unificadas_cia c
                                    where  b.cod_emp = c.cod_emp
                                    and b.cod_unidade =c.cod_unidade_para)
                    group by c.cod_grupo,c.cod_quebra, b.cod_unidade;
               ...
               begin
                    select sum(decode(a.cod_oper,300,0,4300,0,6100,0,nvl(a.vlr_operacao,0) - nvl(a.vlr_entrada,0))) vlr_venda
                           ,count(distinct (decode(a.cod_oper,302,a.num_seq,305,a.num_seq,4302,a.num_seq,4305,a.num_seq))) qtd_venda
                           ,nvl(sum(decode(trim(b.num_modelo),'90',nvl(a.vlr_operacao,0),0)),0) vlr_seguro
                           ,count(distinct (decode(a.cod_oper,6100,a.num_seq))) qtd_seguro
                    into wVendaMesAnt
               ...
               begin
                    ...
                    and nc.num_apolice = 0000100
                    and t.cod_unidade =	 r_seguro.cod_unidade
                    and trunc(dta_cancelamento) >= to_date(pi_dta_iniMes)
                    and trunc(dta_cancelamento) <= to_date(pi_dta_fimMes);
                    exception
                             when no_data_found then
               ...
               begin
                    ...
                    from (select sum(nvl(e.vlr_operacao,0)) vlr_operacao,
                                 ...
                                 e.num_seq, e.cod_maquina, e.cod_oper
                          from ns_notas_operacoes e
                          where e.cod_oper in (3000,3050,6110) --Se colocar so cod_oper do cpp da erro
           ...
           begin
            ...
            where a.num_seq      = b.num_seq
             ...
             and b.cod_unidade  = c.cod_unidade
               and c.cod_grupo    = pi_grupo
             and b.cod_unidade= r_seguro.cod_unidade
             and b.dta_emissao >= to_date(pi_dta_iniAno)
             and b.dta_emissao <= to_date(pi_dta_fimAno)
			 and not exists (select 1 from  grz_lojas_unificadas_cia  c
									 where  b.cod_emp = c.cod_emp
                   and b.COD_UNIDADE =c.COD_UNIDADE_PARA
									 )
            ...
            begin
                Select nvl(sum(nvl(nc.vlr_ressarcido,0)),0) vlr_cancelamento
				 ...
				 into wVlr_cancelamentoAnoAnt
					...
					and nc.num_apolice = 0000100
                    and t.cod_unidade =	 r_seguro.cod_unidade
					and trunc(dta_cancelamento) >= to_date(pi_dta_iniAno)
					and trunc(dta_cancelamento) <= to_date(pi_dta_fimAno)
