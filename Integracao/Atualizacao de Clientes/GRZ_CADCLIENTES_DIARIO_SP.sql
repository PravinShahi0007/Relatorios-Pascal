CREATE OR REPLACE PROCEDURE GRZ_CADCLIENTES_DIARIO_SP
--CREATE OR REPLACE PROCEDURE GRZCADCLIENTESDIARIOSP_TESTE
IS
BEGIN
DECLARE
    iCount                  NUMBER;
    ...
    r_clientes_diario c_clientes_diario%ROWTYPE;
	  ...
	  select * from ai_ps_fisicas a
       where a.dta_transacao = r_clientes_diario.dta_mvto
     ...
     if ((nvl(wCod_Reduzido,0) > 0) and (wAtividade_Cli = '315')) or (nvl(wCod_Reduzido,0) = 0) then
       if (nvl(wCod_Completo,0) > 99) then
      ...
      --###########################
       ...
       BEGIN
                  INSERT INTO AI_PS_PESSOAS (COD_GU,COD_PESSOA,DTA_TRANSACAO,DES_PESSOA,COD_REGIAO,
                                     ...
                                     VALUES (1,wCod_Completo,wDta_mvto,wDes_pessoa,wCod_regiao,
                                             ...
                                             r_clientes_diario.ind_mala_direta,r_clientes_diario.ind_inativo,1,1,
       ...
       if wErro = 0 then
                  ...
                  --###########################
                   ...
                   if wErro = 0 then
                      ...
                      if (nvl(r_clientes_diario.cod_cidade,0) = 44) then
                          ...
                          INSERT INTO ai_ps_pessoas_r900 (COD_EMP,COD_PESSOA,DTA_TRANSACAO,IND_VALIDA_UNIDADE,IND_ATUALIZA_BLOQUEIO
        ...
        if wErro = 0 then
		   ...
		   wPI_WHERE := '';
           wPI_OPCAO := '50#'||wCod_Completo||'#'||wCod_Completo||'#'||to_char(wDta_mvto,'dd/mm/yyyy')||'#'||to_char(wDta_mvto,'dd/mm/yyyy')||'#'||'1#0#1#0#0';
           wPO_ERRO  := '';
           ...
           begin
              AI_PS_IMP_TABELA_SP(wPI_WHERE,wPI_OPCAO,wPO_ERRO);
           end;
           ...
           if r_clientes_diario.tip_pessoa = 1 then
              ...
              BEGIN		   
		          wPI_WHERE := '';
                  wPI_OPCAO := '50#'||r_geracao_fisicas.cod_pessoa||'#'||r_geracao_fisicas.cod_pessoa||'#'||to_char(wDta_mvto,'dd/mm/yyyy')||'#'||to_char(wDta_mvto,'dd/mm/yyyy')||'#'||'1#0#1#0#0';
                  wPO_ERRO  := '';
              ...
              BEGIN		   
		          wPI_WHERE := '';
                  wPI_OPCAO := '50#'||r_geracao_juridicas.cod_pessoa||'#'||r_geracao_juridicas.cod_pessoa||'#'||to_char(wDta_mvto,'dd/mm/yyyy')||'#'||to_char(wDta_mvto,'dd/mm/yyyy')||'#'||'1#0#1#0#0';
                  wPO_ERRO  := '';
      ...
      END;
END GRZ_CADCLIENTES_DIARIO_SP;
--END GRZCADCLIENTESDIARIOSP_TESTE;
